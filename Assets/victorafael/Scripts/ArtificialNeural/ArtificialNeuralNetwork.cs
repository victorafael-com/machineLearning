...
public class ArtificialNeuralNetwork {
	...
	public ArtificialNeuralNetwork(int inputCount, int outputCount, int hiddenLayers, int neuronsPerLayer, double alpha){
		...
		if (hiddenLayers > 0) {
			layers [0] = new NeuronLayer (neuronsPerLayer, inputCount);
			for (int i = 1; i < hiddenLayers; i++) {
				layers [i] = new NeuronLayer (neuronsPerLayer, neuronsPerLayer);
			}
			layers [hiddenLayers] = new NeuronLayer (outputCount, neuronsPerLayer);
		} else {
			layers [0] = new NeuronLayer (outputCount, inputCount);
		}
	}
